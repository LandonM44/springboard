...
class MessageViewTestCase(TestCase):
    ...
    def test_add_like(self):
        m = Message(id=1984, text="Hello World", user_id=self.u1_id)
        db.session.add(m)
        db.session.commit()
    ...
    def test_authenticated_like(self):
        ...
        with self.client as c:
            ...
            self.assertIn("you do not have access to do this.", str(resp.data))
    ...
    def test_unauthorized_following_page_access(self):
        ...
        with self.client as c:
            ...
            resp = c.get(f"/users/{self.testuser_id}/following", follow_redirect=True)
            self.assertEqual(resp.status_code, 200)
            self.assertNotIn("@abc", str(resp.data))
            self.assertIn("you do not have access to do this.", str(resp.data))
